...
class Invoice extends MX_Controller
{
    ...
    public function bdtask_invoice_details($invoice_id = null)
    {
        ...
        $data = array(
            ...
            'total_amount' => number_format($invoice_detail[0]['total_amount'] !== null ? $invoice_detail[0]['total_amount'] : 0, 2, '.', ','),
            'subTotal_quantity' => $subTotal_quantity,
            'total_discount' => number_format($invoice_detail[0]['total_discount'] !== null ? $invoice_detail[0]['total_discount'] : 0, 2, '.', ','),
            'total_discount_cal' => $invoice_detail[0]['total_discount'],
            'total_vat'         => number_format($invoice_detail[0]['total_vat_amnt'] !== null ? $invoice_detail[0]['total_vat_amnt'] : 0, 2, '.', ','),
            'total_tax'         => number_format($invoice_detail[0]['total_tax'] !== null ? $invoice_detail[0]['total_tax'] : 0, 2, '.', ','),
    ...
    public function bdtask_invoice_pad_print($invoice_id)
    {
        ...
        $data = array(
            ...
            'subTotal_quantity' => $subTotal_quantity,
            'total_vat'        => number_format($invoice_detail[0]['total_vat_amnt'] !== null ? $invoice_detail[0]['total_vat_amnt'] : 0, 2, '.', ','),
            'invoice_discount' => number_format($invoice_detail[0]['invoice_discount'] !== null ? $invoice_detail[0]['invoice_discount'] : 0, 2, '.', ','),
            'total_discount'   => number_format($invoice_detail[0]['total_discount'] !== null ? $invoice_detail[0]['total_discount'] : 0, 2, '.', ','),
            'total_tax'        => number_format($invoice_detail[0]['total_tax'] !== null ? $invoice_detail[0]['total_tax'] : 0, 2, '.', ','),
            'subTotal_ammount' => number_format($subTotal_ammount !== null ? $subTotal_ammount : 0, 2, '.', ','),
            'paid_amount'      => number_format($invoice_detail[0]['paid_amount'] !== null ? $invoice_detail[0]['paid_amount'] : 0, 2, '.', ','),
    ...
    public function bdtask_invoice_pos_print($invoice_id = null)
    {
        ...
        $data = array(
            ...
            'subTotal_cartoon'     => $subTotal_cartoon,
            'subTotal_quantity'    => $subTotal_quantity,
            'invoice_discount'     => number_format((int)$invoice_detail[0]['invoice_discount'], 2, '.', ','),
            'total_discount'       => number_format((int)$invoice_detail[0]['total_discount'], 2, '.', ','),
            'total_vat'            => number_format((int)$invoice_detail[0]['total_vat_amnt'], 2, '.', ','),
            'total_tax'            => number_format((int)$invoice_detail[0]['total_tax'], 2, '.', ','),
            'subTotal_ammount'     => number_format((int)$subTotal_ammount, 2, '.', ','),
            'paid_amount'          => number_format((int)$invoice_detail[0]['paid_amount'], 2, '.', ','),
            'due_amount'           => number_format((int)$invoice_detail[0]['due_amount'], 2, '.', ','),
            'shipping_cost'        => number_format((int)$invoice_detail[0]['shipping_cost'], 2, '.', ','),

            'invoice_all_data'     => $invoice_detail,
            'previous' => number_format((int)$invoice_detail[0]['prevous_due'], 2, '.', ','),
            'is_discount'          => $is_discount,
            'is_dis_val'           => $is_dis_val,
            'vat_amnt_per'         => $vat_amnt_per,
            'vat_amnt'             => $vat_amnt,
    ...
    public function invoice_pdf_generate($invoice_id = null)
    {
        ...
        $data = array(
            ...
            'final_date'        => $invoice_detail[0]['final_date'],
            'invoice_details'   => $invoice_detail[0]['invoice_details'],
            'total_amount'      => number_format($invoice_detail[0]['total_amount'] ?? 0 + $invoice_detail[0]['prevous_due'], 2, '.', ','),
            'subTotal_quantity' => $subTotal_quantity,
            'total_discount'    => number_format($invoice_detail[0]['total_discount'] ?? 0, 2, '.', ','),
            'total_vat'         => number_format($invoice_detail[0]['total_vat_amnt'] ?? 0, 2, '.', ','),
            'total_tax'         => number_format($invoice_detail[0]['total_tax'] ?? 0, 2, '.', ','),
            'subTotal_ammount'  => number_format($subTotal_ammount ?? 0, 2, '.', ','),
            'paid_amount'       => number_format($invoice_detail[0]['paid_amount'] ?? 0, 2, '.', ','),
            'due_amount'        => number_format($invoice_detail[0]['due_amount'] ?? 0, 2, '.', ','),
            'previous'          => number_format($invoice_detail[0]['prevous_due'] ?? 0, 2, '.', ','),
            'shipping_cost'     => number_format($invoice_detail[0]['shipping_cost'] ?? 0, 2, '.', ','),
            'invoice_all_data'  => $invoice_detail,
            'company_info'      => $company_info,
            'currency'          => $currency_details[0]['currency'],
            'position'          => $currency_details[0]['currency_position'],
    ...
    public function bdtask_gui_pos()
    {
        ...
        $data['customer_id']   = !empty($walking_customer[0]['customer_id']) ? $walking_customer[0]['customer_id'] : null;
        $data['customer_name'] = !empty($walking_customer[0]['customer_name']) ? $walking_customer[0]['customer_name'] : '';
        ...
        $data['customer_name'] = !empty($customer_details[0]['customer_name']) ? $customer_details[0]['customer_name'] : '';
        $data['customer_id']   = !empty($customer_details[0]['customer_id']) ? $customer_details[0]['customer_id'] : null;
    ...
    {
        // Ensure that the correct HTTP method is used (GET)
        if ($this->input->method() === 'get') {
            $this->db->select_max('invoice', 'invoice_no');
            $query = $this->db->get('invoice');
            $result = $query->row_array(); // Use row_array for a single record

            $invoice_no = isset($result['invoice_no']) ? $result['invoice_no'] + 1 : 1000;
    ...
    //         $query      = $this->db->get('invoice');
    //         $result     = $query->result_array();
    //         $invoice_no = $result[0]['invoice_no'] ?  0; // Handle empty array safely
    //         echo $invoice_no ? $invoice_no + 1 : 1000;
    //     } else {
    //         show_error('Request method not supported', 501);
    //     }
    // }
