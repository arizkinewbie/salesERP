...
class Invoice_model extends CI_Model
{
    ...
    public function pos_customer_setup()
    {
        return [['customer_name' => 'Walking Customer', 'customer_id' => '1']];
    }
    ...
    public function getInvoiceList($postData = null)
    {
        ...
        $datbetween = (!empty($fromdate) && !empty($todate)) ? "a.date BETWEEN '$fromdate' AND '$todate'" : "";
        ...
        $searchQuery = "";
        if ($searchValue != '') {
            $searchQuery = "(b.customer_name LIKE '%" . $searchValue . "%' 
        ...
        $this->db->group_by('a.invoice_id'); // Ensures unique invoices
        ...
        foreach ($records as $record) {
            ...
            $button .= '&nbsp;<a href="' . $base_url . 'invoice_pad_print/' . $record->invoice_id . '" class="btn btn-primary btn-sm" 
                        data-toggle="tooltip" data-placement="left" title="' . display('pad_print') . '">
                        <i class="fa fa-fax" aria-hidden="true"></i></a>';

            $button .= '&nbsp;<a href="' . $base_url . 'pos_print/' . $record->invoice_id . '" class="btn btn-warning btn-sm" 
                        data-toggle="tooltip" data-placement="left" title="' . display('pos_invoice') . '">
                        <i class="fa fa-fax" aria-hidden="true"></i></a>';
            ...
            if ($this->permission1->method('manage_invoice', 'update')->access()) {
                ...
                if ($approve == 0 && empty($record->ret_adjust_amnt)) {
                    $button .= '&nbsp;<a href="' . $base_url . 'invoice_edit/' . $record->invoice_id . '" class="btn btn-info btn-sm" 
        ...
        if ($multipaytype && $multipayamount) {
            ...
            if ($multipaytype[0] == 0) {
                ...
                if ($subcode) {
                    ...
                    $subcode_id = null;  // Or set a default value or log an error
                }
    ...
    public function insert_sale_creditvoucher($is_credit = null, $invoice_id = null, $dbtid = null, $amnt_type = null, $amnt = null, $Narration = null, $Comment = null, $reVID = null, $subcode = null)
    {
        ...
        // Ensure $subcode is a scalar value
        if (is_object($subcode)) {
            $subcode = $subcode->id; // Extract the required field from the object.
        }
        ...
        // dd($debitinsert); // Ensure all data is scalar.
    ...
    //POS invoice entry
    public function pos_invoice_setup($productId)
    {
    ...
    public function bdtask_invoice_pos_print_direct($invoice_id = null)
    {
        ...
        $data = array(
            ...
            'all_discount' => number_format($invoice_detail[0]['total_discount'] !== null ? $invoice_detail[0]['total_discount'] : 0, 2, '.', ','),
